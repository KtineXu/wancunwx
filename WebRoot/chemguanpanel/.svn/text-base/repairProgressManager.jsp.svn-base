...
<div class="widget-box">
          ...
          <form action="behindDeleteProgress.do" method="post">
        ...
        <c:forEach var="pro" items="${proList}">
		      ...
		      <td><input type="checkbox" value="${pro.id}" name="map" /></td>
		      <td>${pro.id}</td>
		      <td>
		      		<c:set var="string1" value="${pro.addDate}"/>
		      		<c:set var="string2" value="${fn:substring(string1,0,19)}"/>${string2 }
		      </td>
		      <td>${pro.description} </td>
		     <td style="text-align: center;"><a href="addRepairProgressUI.do?repairId=${repairId}">添加维修进度</a></td> 
     </tr>
     </c:forEach>
            </table>
            </div>
