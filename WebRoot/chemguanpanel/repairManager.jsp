...
<div class="widget-box">
          ...
          <form action="behindDeleteRecords.do" method="post">
        ...
        <c:forEach var="r" items="${ruList}">
		      <tr style="font-size: 12px;">
		      <td><input type="checkbox" value="${r.repairRecords.id}" name="map" /></td>
		      <td>${r.repairRecords.id}</td>
		      <td>${r.repairRecords.productName}</td>
		      <td>${r.repairRecords.courierName} </td>
		      <td>${r.repairRecords.courierNumber}</td>
		      <td>
		      	<c:set var="string1" value="${r.repairRecords.addDate}"/>
		      	<c:set var="string2" value="${fn:substring(string1,0,19)}"/>${string2 }
		      </td>
		       <td>${r.repairRecords.remark}</td>
		       <td>${r.users.realName}</td>
		     <td style="text-align: center;"><a href="behindFindProgress.do?repairId=${r.repairRecords.id }">查询维修进度</a>
     </tr>
     </c:forEach>
            </table>
            </div>
